#!/usr/bin/env node
// QVAC Fake History Fact Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29359;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/fakehistory") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake History Fact Generator GUI ready at http://localhost:${PORT}`);
  });
